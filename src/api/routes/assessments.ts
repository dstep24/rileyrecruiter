...
import { aiAssessmentGenerator } from '../../domain/services/AIAssessmentGenerator.js';
...
router.post('/templates', async (req: Request, res: Response) => {
  try {
    const { tenantId, name, description, roleType, companyName, questions } = req.body as {
      tenantId?: string;
      name: string;
      description?: string;
      roleType?: string;
...
// =============================================================================
// AI ASSESSMENT GENERATION (Authenticated)
// =============================================================================

/**
 * Generate assessment from custom job context (no DB record required)
 * POST /api/assessments/generate-from-context
 *
 * Creates a tailored pre-screening assessment based on provided job context.
 * Useful when sourcing with custom JD parsing (no job requisition in DB).
 */
router.post('/generate-from-context', async (req: Request, res: Response) => {
  try {
    const { title, description, requirements, preferredSkills, location, locationType, companyName, tenantId } = req.body as {
      title: string;
      description: string;
      requirements?: string[];
    ...
    // Build job context from the provided data
    const jobContext = {
      title,
      description,
    ...
    // Generate assessment using the AI generator
    const assessment = await aiAssessmentGenerator.instance.generateAssessment(jobContext);

    // Save the template to database (without job requisition linkage)
    const template = await prismaBase.preScreeningTemplate.create({
      data: {
        ...
        companyName: companyName,  // Store company name for internal tracking (NOT shown to candidates)
    ...
    console.log('[Assessments] Generated assessment from context:', template.id);
    ...
    console.error('[Assessments] Generate from context error:', error);
    res.status(500).json({ success: false, error: 'Failed to generate assessment' });
  }
});

/**
 * Generate assessment from a job requisition
 * POST /api/assessments/generate-from-job
 *
 * Creates a tailored pre-screening assessment based on the job description.
 * If an assessment already exists for this job, returns the existing one.
 */
router.post('/generate-from-job', async (req: Request, res: Response) => {
  try {
    const { jobRequisitionId, tenantId } = req.body as {
    ...
    if (!jobRequisitionId) {
      return res.status(400).json({
        success: false,
        error: 'jobRequisitionId is required',
      });
    }

    const effectiveTenantId = tenantId || 'development';

    const result = await aiAssessmentGenerator.getOrGenerateAssessment(
      jobRequisitionId,
      effectiveTenantId
    );

    // Get the full template with questions
    const template = await preScreeningService.getTemplate(result.templateId);
    ...
    console.error('[Assessments] Generate from job error:', error);
    res.status(500).json({ success: false, error: 'Failed to generate assessment' });
  }
});

/**
 * Get assessment for a job requisition
 * GET /api/assessments/job/:jobRequisitionId
 */
router.get('/job/:jobRequisitionId', async (req: Request, res: Response) => {
  try {
    const jobRequisitionId = Array.isArray(req.params.jobRequisitionId)
      ? req.params.jobRequisitionId[0]
      : req.params.jobRequisitionId;

    const template = await prismaBase.preScreeningTemplate.findFirst({
      where: {
        jobRequisitionId,
        isActive: true,
      },
    ...
    // Generate a shareable link URL (without a specific conversation)
    const baseUrl = process.env.DASHBOARD_URL || 'http://localhost:3001';

    res.json({
      success: true,
      template,
      // Note: To get a specific assessment link, use POST /api/assessments/send
      shareableTemplateUrl: `${baseUrl}/assessments?templateId=${template.id}`,
    });
  } catch (error) {
    console.error('[Assessments] Get job assessment error:', error);
    res.status(500).json({ success: false, error: 'Failed to get assessment' });
  }
});

/**
 * Create assessment link for job + candidate (for messaging queue)
 * POST /api/assessments/create-link-for-job
 *
 * This is used when adding candidates to the outreach queue - it creates
 * a personalized assessment link that can be embedded in the outreach message.
 */
router.post('/create-link-for-job', async (req: Request, res: Response) => {
  try {
    const { jobRequisitionId, conversationId, candidateName, candidateEmail, tenantId } = req.body as {
      jobRequisitionId: string;
    ...
    if (!jobRequisitionId || !conversationId) {
      return res.status(400).json({
        success: false,
        error: 'jobRequisitionId and conversationId are required',
      });
    }

    const effectiveTenantId = tenantId || 'development';

    // Get or create assessment template for this job
    const { templateId, isNew } = await aiAssessmentGenerator.getOrGenerateAssessment(
      jobRequisitionId,
      effectiveTenantId
    );

    // Create the assessment link for this specific candidate
    const assessmentLink = await preScreeningService.createAssessmentLink({
      templateId,
      conversationId,
      candidateName,
      candidateEmail,
    ...
    console.error('[Assessments] Create link for job error:', error);
